...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, uint32_t M,
  ...
  size_t x_base = blockIdx.x * blockDim.x + threadIdx.x;
  size_t y_base = blockIdx.y * blockDim.y + threadIdx.y;
  if (x_base < M && y_base < P) {
    scalar_t sum = 0;
    for (uint32_t k = 0; k < N; ++k)
      sum += a[x_base * N + k] * b[k * P + y_base];
    out[x_base * P + y_base] = sum;
  }
}
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  /// BEGIN YOUR SOLUTION
  CudaDims dim;
  dim.grid = dim3(THREAD_NUM_X, THREAD_NUM_Y, 1);
  dim.block = dim3((M + THREAD_NUM_X - 1) / THREAD_NUM_X, (P + THREAD_NUM_Y - 1) / THREAD_NUM_Y, 1);
